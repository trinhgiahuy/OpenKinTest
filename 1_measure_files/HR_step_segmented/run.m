...
db_oxy = containers.Map({'1','2','3','4','5','7','8', ...
                    ...
                    {'OXYGA1 marker 0120_s','OXYGA2 marker 0035_s','OXYGA3 marker 0110_s', ...
                     ...
                     'OXYGA9 marker 0102','OXYGA10 marker 0129','OXYGA11 marker 0059','OXYGA12 marker 0051','OXYGA13 marker 0137'});
db_gaitpod = containers.Map({'1','2','3','4','5','7','8', ...
                        '9','10','11','12','13'}, ...
                            {'2022-05-23-08-06-47_s','2022-05-23-08-50-25_s','2022-05-23-09-32-52_s', ...
                             ...
                             '2022-11-01-08-32-09','2022-11-01-09-37-37','2022-11-01-11-11-18','2027-09-01-23-42-34','2022-11-02-07-14-20'});
...
db_HR = containers.Map({'9','10','11','12','13'}, ...
                        {'2022-11-01_09-25-19_203130000581_HeartRateTestActivity_s' ...
                          '2022-11-01_10-08-41_203130000581_HeartRateTestActivity_s' ...
                          '2022-11-01_11-44-03_203130000581_HeartRateTestActivity_s' ...
                          '2022-11-01_12-21-31_203130000581_HeartRateTestActivity_s' ...
                          '2022-11-02_07-51-37_203130000581_HeartRateTestActivity'});
%% Load HR file
HR_fn=[db_HR(num2str(subjectId)),'.csv'];
%HR_fn="2022-11-01_10-08-41_203130000581_HeartRateTestActivity.csv";
%HRopts = detectImportOptions(HR_fn);
%HRopt.DataLines = 1;
%HRopts.VariableNamesLine = 1;
%HR_data_table = readtable(HR_fn,HRopts);

%%HR linear here is sampling at 400Hz
HR_linear = linearInter(HR_fn);
%HR_linear=HR_linear(HR_fn);
HR_linear=HR_linear.';
...
hr_idx=(1:2000:length(HR_linear));
hr_match = zeros(size(hr_idx));
for i=1:length(hr_idx)-1
    hr_match(i)=mean(HR_linear(hr_idx(i):hr_idx(i+1)-1));
end
...
yyaxis right
plot(VO2_in_sgolay(1,1:length(VO2_in_sgolay)),'-k')
...
legend('Longitudinal velocity INS','VO2 Uptake Original','VO2 Uptake per kg smooth[ml/min/kg]','Heart Rate','location','best')
...
%%Enable this after finish slicing
if (subjectId>9)
    csvwrite(['O',oxygenID(6),oxygenID(7),'_output_smooth.csv'],VO2_in_sgolay(1,:).');
else
    csvwrite(['O',oxygenID(6),'_output_smooth.csv'],VO2_in_sgolay(1,:).');
end
...
%HR_linear_rep = repelem(HR_linear,400);
disp(size(HR_linear));
cell_array_parser_vn200(fileID,subjectId,'vn200',size(HR_linear));
...
disp('Call velSeg()...');
[windowEnd_r,back_r,stepCount]=velSeg(subjectId,HR_linear);
disp('Finish velSeg()')

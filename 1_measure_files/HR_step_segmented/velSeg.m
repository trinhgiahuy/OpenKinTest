function [windowEnd_r,back_r,stepCount] = velSeg(subjectId,HR_linear)
...
for i = 1:stepCount
    ...
    %time_hr(i)=steps(i)/400;  % I think 1 here is rate of HR output same as 400 above
    %k_ = floor(time_hr(i)/1)+1;
    %HRTest = [HRTest, HR_data(k_)];
...
save(fileName,'steps', 'speed', 'speedChange', 'stepDuration', 'vertOscillation_dist_amp','HRTest','oxyTest');
end

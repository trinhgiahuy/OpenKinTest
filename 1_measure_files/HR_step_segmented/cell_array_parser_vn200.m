function [] = cell_array_parser_vn200(fileID,subjectId, vn200, HRLinearRepSize)
...
if (vn200)
    ...
    disp(size(v_n));
    disp(HRLinearRepSize);
    %% SLICING GAITPOD DATA SO THAT EQUAL TO HEART RATE
    v_n=v_n(1:HRLinearRepSize);
    v_e=v_e(1:HRLinearRepSize);
    v_d=v_d(1:HRLinearRepSize);

    %for k=1:max(size(T))
    for k=1:HRLinearRepSize

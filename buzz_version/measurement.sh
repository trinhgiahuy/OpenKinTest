...
#echo $BUZZGPIO[0] #ouput 21[0]
#echo ${BUZZGPIO[0]} #output 21

logger "Start buzzer controller"
if [ "${BUZZ[0]}" -eq 4 ]; then
    sudo bash $HOME/OldGaitMaven/openkin/buzz-linuxgpio.sh 0 ${BUZZGPIO[0]} >> $BUZZLOG 2>&1 &
    #sudo bash $HOME/OldGaitMaven/openkin/buzz-linuxgpio.sh 0 ${BUZZGPIO[0]} 
    
    #sudo echo "ON" | sudo tee /tmp/buzzpipe0
    #sleep 5
    #sudo echo "OFF" | sudo tee /tmp/buzzpipe0
    #sleep 5
    ...
    echo $!
    sleep 1
    BUZZPID[0]=$(ps --ppid ${SUDOPID[4]} -o pid=)
    echo "${BUZZPID[$i]}"
    logger "Buzzer connected"
    sleep 2
    buzz_on 0
    sleep 1
    buzz_off 0
...
function quitScreens {
	...
	#FIX: ONLINE-SHUTDOWN-LOGGER--TURN OFF BUZZ
	buzz_off 0
...
#	logger "Running loop"
	...
	if [ "$online" -eq 0 ]; then
	    #   ONLINE - TURN OFF BUZZ
	    #    echo "OFF" > $BUZZRECORDING
...
'
			if ! (screen -list | grep -q "log") && [[ $IMUERR -eq 0 ]] && [[ $MTWERR -eq 0 ]] && [[ $GPSERR -eq 0 ]] && [[ $INSERR -eq 0 ]] && [[ $POZYXERR -eq 0 ]]; then
				...
				logger "Call ascii_logger listener.py > ASCIILOG file"
				led_blink 2
				led_blink 0
				echo "ON" > $INSRECORDING

				#buzzer call func here
				# buzz_pulse 2
				# TEST: buzz_pulse 0
				buzz_pulse 0
				echo "ON" > $BUZZRECORDING
			fi

...
/*  int ret = wiringPiI2CSetup(0x4d);
  if (ret != -1) {
    std::cout << "wiringPi loaded" << std::endl;

    int data = wiringPiI2CRead(ret);
    std::cout << "Read: " << data << std::endl;
  } else {
    std::cerr << "wiringPi error: " << ret << std::endl;
  }
*/
  if(Pozyx.begin(true) == POZYX_FAILURE){
    std::cout << "Connection to Pozyx failed!" << std::endl;
  ...
  std::cout << "Out of setup!" << std::endl;
...
//  Serial.println();             
  std::cout << ",";
//  std::cout calib_status&0x03;
  std::cout << ",";
//  std::cout << (calib_status&0x0C)>>2;
  std::cout  << ",";
//  std::cout << (calib_status&0x30)>>4;
  std::cout << ",";
//  std::cout << (calib_status&0xC0)>>6;
}
